...
public class GridStatistics extends RegistryBasedServlet {
    ...
    private String getGridLoadResponse() {
        StringBuffer stringBuffer = new StringBuffer();
        BrowserStatisticsCollection gridStatistics = getBrowserMaxStatistics();
        updateWaitingRequests(gridStatistics);
        stringBuffer.append(gridStatistics.toString());
        return stringBuffer.toString();
    }

    private BrowserStatisticsCollection getBrowserMaxStatistics() {
        BrowserStatisticsCollection gridStatistics = new BrowserStatisticsCollection();
        BrowserInformationCache nodeBrowserCapacity = BrowserInformationCache.getInstance();
        for (String browserName : BrowserInformationCache.SUPPORTED_BROWSERS) {
            int totalBrowserCapacity = nodeBrowserCapacity.getTotalBrowserCapacity(browserName, getRegistry());
            if (totalBrowserCapacity > 0) {
                gridStatistics.setMaxBrowserInstances(browserName, totalBrowserCapacity);
            }
        }
